/**
 * Ejemplo de uso de Fetchly.
 * Muestra los 5 métodos HTTP (GET, POST, PUT, PATCH, DELETE), el sistema
 * de eventos, y cómo consumir la librería con async/await y con Promises.
 */
import { FetchlyClient } from './src/core/index';

/**
 * Estructura de un usuario devuelto por la API de ejemplo
 */
interface Usuario {
  id: number;
  name: string;
...
client.events.on('onTimeout', (data) => {
  console.error(`Timeout: ${data.method} ${data.url} superó ${data.timeout}ms`);
});

/**
 * Ejecuta los 5 métodos HTTP de Fetchly usando async/await.
 */
...
/**
 * Realiza una petición GET usando el manejo de Promises (.then/.catch)
